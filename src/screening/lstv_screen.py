    ...
    if series_df is not None and study_id is not None:
        ...
        if len(study_series) > 0:
            ...
            for priority in priorities:
                matching = study_series[study_series['series_description'].str.contains(priority, case=False, na=False)]
                if len(matching) > 0:
...
def convert_dicom_to_nifti(dicom_dir, output_path):
    try:
        ...
        cmd = ['dcm2niix', '-z', 'y', '-f', bids_base, '-o', str(output_path.parent), '-m', 'y', '-b', 'n', str(dicom_dir)]
...
def run_spineps_inference(nifti_path, output_dir):
    try:
        ...
        cmd = ['bash', '/work/src/screening/spineps_wrapper.sh', 'sample', '-i', str(nifti_path),
               '-model_semantic', 't2w', '-model_instance', 'instance', '-model_labeling', 't2w_labeling',
...
def analyze_segmentation(seg_path):
    try:
        ...
        return {
            'vertebra_count': vertebra_count, 'has_sacrum': has_sacrum, 'has_l6': has_l6,
            's1_s2_disc': s1_s2_disc, 'is_lstv_candidate': is_lstv, 'lstv_type': lstv_type,
            'unique_labels': list(map(int, unique_labels)), 'lumbar_labels': list(map(int, lumbar_labels)),
        }
    except:
        return None
...
def upload_to_roboflow(image_path, study_id, roboflow_key, workspace, project):
    try:
        ...
        project_obj.upload(image_path=str(image_path), split="train", tag_names=["lstv-candidate", "automated"], num_retry_uploads=3)
        return True
    except:
        return False
...
def main():
    ...
    for study_dir in tqdm(study_dirs, desc="Processing"):
        ...
        try:
            ...
            result = {
                'study_id': study_id, 'series_id': series_dir.name,
            ...
            if analysis['is_lstv_candidate']:
                ...
                if extract_middle_slice(nifti_path, image_path):
                    if upload_to_roboflow(image_path, study_id, args.roboflow_key, args.roboflow_workspace, args.roboflow_project):
                        print(f"  ✓ Uploaded")
